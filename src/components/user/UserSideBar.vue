<template>
  ...
  >
    ...
    <div class="hidden md:block pb-6 mb-6 border-b border-golden-brown/50">
      <router-link :to="{ name: 'Home' }" class="text-bone-white text-2xl font-bold tracking-wide" :style="{ fontFamily: 'var(--font-display)' }">
        Chuvi <span class="text-golden-brown">User</span>
      </router-link>
    </div>

    <!-- Navigation Links -->
    <nav class="space-y-1 flex-grow">
      <h3 class="text-cream text-sm uppercase font-semibold mb-3 tracking-wider">Account</h3>
      ...
      >
        <font-awesome-icon 
          ...
          :class="isActive(item.routeName) ? 'text-navy-blue' : 'text-golden-brown group-hover:text-golden-brown'"
        />
        <span class="text-base">{{ item.name }}</span>
      </router-link>

      <div class="pt-6 mt-6 border-t border-golden-brown/50">
        <h3 class="text-cream text-sm uppercase font-semibold mb-3 tracking-wider">Services</h3>
        ...
        >
          <font-awesome-icon icon="calendar-plus" class="w-5 h-5 mr-3 text-pure-gold" :class="{'!text-navy-blue': $route.name === 'BookPickup'}" />
    ...
    <div class="pt-8 mt-auto border-t border-golden-brown/50">
      ...
      >
        <font-awesome-icon icon="sign-out-alt" class="w-5 h-5 mr-3 text-red-400" />
        Log Out
      </button>
    </div>
  </aside>
